...
#lmf
par(mar=c(0,0,0,0))
with(fracalloc, plot(lmf~standingtreemass, ylab="", axes=FALSE,xlab="", xlim=c(5000,22500), ylim=c(0,.3), type='n'))
ablineclip(lmfmod, x1=min(fracalloc$standingtreemass), x2=max(fracalloc$standingtreemass),lwd=2)
with(fracalloc, points(lmf~standingtreemass, pch=c(1,19)[Water_treatment],col=CO2_treatment, cex=1.5))
...
text(x=5250, y=0.29,labels="(a)", cex=1.5)
box()
...
axis(1, labels=FALSE, tcl=.5)
axis(4, at=at.yleaf,labels=TRUE)
box()
text(x=7800, y=0.29,labels="(b)", cex=1.5)

#smf
par(mar=c(0,0,0,0))
with(fracalloc, plot(smf~standingtreemass,  ylab="", xaxt='n', yaxt='n', xlab="",ylim=c(.25,.7), xlim=c(5000,22500),type='n'))
ablineclip(smfmod, x1=min(fracalloc$standingtreemass), x2=max(fracalloc$standingtreemass),lwd=2)
with(fracalloc, points(smf~standingtreemass, pch=c(1,19)[Water_treatment],col=CO2_treatment, cex=1.5))
mtext(smflab, side=2, outer=TRUE, line=3.5, at=.5,las=3)
axis(1, labels=FALSE, tcl=.5)
axis(2, at=at.y2 ,labels=TRUE)
text(x=5250, y=0.685,labels="(c)", cex=1.5)

#stem allocation
par(mar=c(0,0,0,0))
with(fracalloc, plot(stemallocation~ cflux11,  yaxt='n', ylab="", xlab="",ylim=c(.25,.7), xlim=c(7500,27500),type='n'))
...
axis(4, labels=TRUE)
box()
text(x=7800, y=0.685,labels="(d)", cex=1.5)

#rmf
par(mar=c(0,0,0,0))
with(fracalloc, plot(rmf~standingtreemass,  ylab="", xaxt='n',yaxt='n',xlab= "",ylim=c(.1, .3), xlim=c(5000,22500),type='n'))
ablineclip(rmfmod, x1=min(fracalloc$standingtreemass), x2=max(fracalloc$standingtreemass),lwd=2)
with(fracalloc, points(rmf~standingtreemass, pch=c(1,19)[Water_treatment],col=CO2_treatment, cex=1.5))
axis(2, labels=TRUE)
