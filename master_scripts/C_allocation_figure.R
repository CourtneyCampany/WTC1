...
##calculated component fractions
  tree_C$lmf <- with(tree_C, (leafC+litterC)/treeC)
  tree_C$smf <- with(tree_C, (boleC+stemC)/treeC)
  tree_C$rmf <- with(tree_C, (frootC + crootC)/treeC)
...
#lmf
par(mar=c(0,7,0,2))
with(tree_C, plot(lmf~treeC, ylab="", xaxt='n', xlab="", xlim=c(5000,20000), ylim=c(.15,.3), type='n'))
...
text(x=20000, y=0.295,labels="(a)", cex=1.5)

#smf
par(mar=c(0,7,0,2))
with(tree_C, plot(smf~treeC,  ylab="", xaxt='n', yaxt='n', xlab="",ylim=c(.35,.75), xlim=c(5000,20000),type='n'))
...
text(x=20000, y=0.73,labels="(b)", cex=1.5)

#rmf
par(mar=c(0,7,0,2))
with(tree_C, plot(rmf~treeC,  ylab="", xaxt='n',yaxt='n',xlab= "",ylim=c(.15, .35), xlim=c(5000,20000),type='n'))
...
text(x=20000, y=0.34,labels="(c)", cex=1.5)
