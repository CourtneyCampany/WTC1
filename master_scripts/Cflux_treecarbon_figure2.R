...
mab_mod <- lm(Cab ~ cflux11, data = treeC11)
# summary(mab_mod)
# anova(mab_mod)
...
#whole tree
par(mar=c(0,0,0,0))
plot(1,type='n', axes=FALSE, ann=FALSE,xlim = c(3000, 30000),ylim = c(3000, 30000))
...
#Mab
par(mar=c(0,0,0,0))
plot(1,type='n', ylab = "",xlab="",xlim = c(3000, 30000),ylim = c(3000, 30000))
mtext(Mablab, side=2,line=4, outer=TRUE, at=.25,las=0, cex=1)
mtext(treefluxlab2, side=1,line=3, outer=TRUE, las=0, cex=1)
box()
ablineclip(mab_mod, x1=min(treeC11$cflux11), x2=max(treeC11$cflux11),lwd=2)
abline(0, 1, lty=3, lwd=2)
axis(1,labels=TRUE, outer=TRUE)
axis(2,labels=TRUE, outer=TRUE)
points(Cab ~ cflux11, data = treeC11,pch=c(1,19)[Water_treatment],col=CO2_treatment, cex=1.5)
text(x=30000, y=27500, labels="(b)", cex=1.25)

# dev.copy2pdf(file= "master_scripts/paper_figs/flux_treecarbon2.pdf")
# dev.off()
