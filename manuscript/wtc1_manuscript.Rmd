...
At the end of this two year experiment, harvested C mass of tissue components was affected by eC~a~ but not drought treatments (Table 1). Aboveground wood C mass was reduced by 37 % under eC~a~ (P = 0.0151), driven mostly by eC~a~ effects on boles. Neither standing crop leaf C mass or total litterfall C mass over the study period were affected by C~a~. Total root C mass was marginally reduced under eC~a~ (P = 0.091).
...
Across all treatment combinations, the total C mass of boles, branches, leaves and roots was on average 61.0&pm;0.02 % of $\text{F}_{c,T}$ (Figure `r additivemassfig`). Additionally, $\text{C}_{ab}$ was on average 41.2&pm;0.004 % of $\text{F}_{c,T}$ on any given day. Across a large range in tree size, similar patterns were detected in each individual WTC (Figure `r additivechamfig`). As mass balance must be achieved, TBCA and $\text{F}_{c,r}$ were estimated from Figure `r additivemassfig` as residuals between $\text{F}_{c,T}$ and whole tree mass excluding and including roots, respectively. Neither cumulative TBCA nor $\text{F}_{c,r}$ were affected by C~a~ or drought treatments (Figure `r belowbarfig`). TBCA and $\text{F}_{c,T}$ were positively correlated over the final 11 months of the experiment (R^2^ = 0.78, P < 0.001) and the proportion of C allocated belowground was relatively constant through time and between treatments (Figure `r tbcafluxfig`).

# Discussion
A whole-tree chamber experiment provided a unique opportunity to study the carbon balance of *Eucalyptus* trees. We found that biomass partitioning and C allocation of component tissues were differentially affected by elevated [CO~2~]. Minimal effects of a four month drought were detected on total tree C flux, biomass partitioning and tissue C allocation, despite previous findings of negative effects of drought on leaf and canopy physiology [see @duursma2011rooting; @crous2012light]. Using a novel methodological framework, we show that TBCA was unchanged by either eC~a~ or drought and remained constant across daily times steps. The consistency of TBCA, at both daily and annual time scales, suggests that TBCA may not be as sensitive to the effects of climate changes as previously assumed.

##Relationships between tree C flux, leaf area and tree C mass##
As expected, tree C uptake and growth were coordinated across this two year experiment. Cumulative tree C flux was positively correlated to both canopy leaf area and total biomass produced. The net C uptake of plants should be a function of the canopy leaf area because leaf area index determines canopy light interception [@wilson1965stand; @monsi2005factor] and is correlated to canopy assimilation and tree productivity [@lindroth2008leaf; @mccarthy2006canopy; @waring1983estimating]. Determining tree canopy C flux, however, is usually inhibited by simple upscaling of single leaf measurements [@amthor1994scaling], oversimplification of big leaf models [@de1997simple] or parameterization of more complex models with assumptions of canopy behavior [@leuning1995leaf]. A novel aspect of this study was the ability to measure whole tree C fluxes directly and compare these fluxes to observed patterns in leaf area and growth. We found that leaf area was consistently reduced under eC~a~, likely leading to reductions in both tree C flux and whole tree C mass of near identical magnitudes (ca. 30 %).  
...
# List of Figures
**Figure 1**. Whole tree C mass as a function of cumulative aboveground C flux, for each WTC, over the final eleven months of the experiment. The dotted line is the 1:1 relationship and the solid lines represent the significant linear model fit from the equation y = 0.56x + 878.2 (R^2^ = 0.86).

\
**Figure 2**. Estimated canopy leaf area for each WTC tree over the final eleven months of the experiment (April 2008 to March 2009). Estimates are based on height growth, litterfall rates and leaf area estimates following Barton et al. [-@barton2012effects]. Color and line type distinguish the treatment combination for each individual chamber.

\
**Figure 3**. Treatment means of cumulative aboveground C flux as a function on mean daily leaf area over the final eleven months of the experiment. The solid line represents the significant linear model fit (R^2^ = 0.77) from the equation: y = 611.9x + 2791.2.
...
source("component script/root_predict.R")
```
\
**Figure S1**.
