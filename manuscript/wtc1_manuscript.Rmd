...
terrestrial biosphere [@iversen2014terrestrial], yet how limitations imposed by drought interact with the growth-stimulating effects of eC~a~ requires more attention [@duursma2011rooting]. 
...
\
(2) At the end of the 2 year experiment we expected partitioning of C to roots to increase under eC~a~, similar to previous studies. We also expected increases in partitioning to roots under drought treatments, as trees should attempt to reduce water limitation.

\
(3) As shifts in partitioning to root biomass were hypothesized, we expected TBCA to increase through time as cumulative tree C flux became affected by eC~a~ and drought. Additionally, we expected C allocation to leaves and woody tissue components aboveground to remain constant.
...
At the end of this two year experiment, harvested C mass of tissue components was affected in eC~a~ but not drought treatments (Table 1). Aboveground wood C mass was reduced by 37 % in eC~a~ treatments (P = 0.0151), driven mostly by eC~a~ effects on boles. Neither standing crop leaf C mass or total litterfall C mass over the study period differed between C~a~ treatments. Total root C mass was marginally reduced in eC~a~ treatments (P = 0.091).

\
Leaf mass fraction (LMF) increased by 15.0 % in eC~a~ treatments (P = 0.011) but was not affected by the drought treatment. Leaf mass fraction was negatively correlated with whole tree C mass (P= 0.007, Figure `r massfracfig`a). Stem mass fraction (SMF) was marginally reduced by 5.8 % under eC~a~ (P = 0.083), with no effect of the drought treatment detected. Stem mass fraction had a weak positive correlation with whole tree C mass (P = 0.09, Figure `r massfracfig`c). Root mass fraction (RMF) was not affected by either treatment and was not correlated to whole tree C mass (Figure `r massfracfig`e).
...
Across all treatment combinations, the total C mass of boles, branches, leaves and roots was on average 61.0&pm;0.02 % of $\text{F}_{c,T}$ (Figure `r additivemassfig`). As mass balance must be achieved, TBCA and $\text{F}_{c,r}$ were estimated from Figure `r additivemassfig` as residuals between $\text{F}_{c,T}$ and whole tree mass excluding and including roots, respectively. Total belowground C allocation was on average 49.9&pm;0.02 of $\text{F}_{c,T}$ and ranged from 46.1 to 54.9 % across treatment combinations. Across a large range in tree size, similar patterns were detected in each individual WTC (Figure `r additivechamfig`). Neither cumulative TBCA nor $\text{F}_{c,r}$ were affected by C~a~ or drought treatments (Figure `r belowbarfig`). TBCA and $\text{F}_{c,T}$ were positively correlated over the final 11 months of the experiment (R^2^ = 0.78, P < 0.001) and the proportion of C allocated belowground was relatively stable through time and between treatments (Figure `r tbcafluxfig`).
