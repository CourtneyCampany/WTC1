...
library(png)
concept <- readPNG("manuscript/conceptfig2.png")
par(mar=c(0,0,0,0))
...
\
A shortage of empirically measured whole tree C fluxes also makes inferring relationships with biomass or C allocation difficult. Biomass and C fluxes have been found to be poorly related in forest ecosystems due to difficulty in accounting for C retention of different tissues [@litton2007carbon]. This partial accounting of C likely inhibits the ability of many studies to precisely test the coordination between canopy A and growth. The advantage of the WTC approach is the ability to compare cumulative whole tree C fluxes to absolute biomass production over long time periods. Here, we show empirically measured aboveground tree C uptake (F~c~) was strongly correlated to tree biomass production across a 2.5 fold size range in *Eucalyptus* trees. 
