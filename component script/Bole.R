...
write.csv(Bole_Mass, file = "calculated mass/bole mass.csv", row.names=FALSE)

#alternatively I can have a file with bark and wood mass seperate
barkandwood <- aggregate(cbind(bark_mass,wood_mass,Volume) ~ Date + chamber, data = stem_mass, FUN = sum)
tmp <- subset(barkandwood, chamber=="ch06")
