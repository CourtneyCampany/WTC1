####problem is that the large values on graph(is 12/09/2013) include extra measurements above 310
#### these measurement are actually really imporatnt because they include all the split stems
#### wiht all the other dates I can only estimate the stemsegment 1 from 310 to height
...
ch11crown <- subset(crown3, chamber =="ch11")
...
#mmm <- subset(basetrunk3, basetrunk3$chamberdate == "ch01;2008-12-09")
...
#test <- subset(trunk_all, trunk_all$chamberdate == "ch01;2008-12-09")


#trunk1 <- subset(trunk, chamberdate == "ch01;2008-04-22") 
#for (i in 1:length(trunk1$CSA)){
  # trunk1$CSAadd[i] <-(trunk1$CSA[i]+trunk1$CSA[i+1])
  #}
# do.call(rbind, trunk1)
...
#test2<- subset(trunkvolume, trunkvolume$chamber== "ch03")
...
#bole_mass_adj <- bole_mass[! (bole_mass %in% c("2008-11-11", "2008-12-23", "2009-01-05", "2009-02-03", "2009-02-17"))]

#plot(bole_mass ~ Date, data=subset(bole_mass_adj, chamber=="ch11"))

#bolemass11 <- subset(bole_mass_adj, chamber = "ch11")
